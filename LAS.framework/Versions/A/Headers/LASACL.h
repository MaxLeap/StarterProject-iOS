...
@class LASUser, LASRole;

/**
 *  A LASACL is used to control which users can access or modify a particular object. Each LASObject can have its own LASACL. You can grant read and write permissions separately to specific users, to groups of users that belong to roles, or you can grant permissions to "the public" so that, for example, any user could read a particular object but only a particular set of users could write to that object.
 */
@interface LASACL : NSObject
 ...
 */
+ (LASACL *)ACLWithUser:(LASUser *)user;
 ...
 */
- (void)setReadAccess:(BOOL)allowed forUser:(LASUser *)user;
...
/*!
 ...
 @return YES if the user has read access. NO otherwise.
 */
- (BOOL)getReadAccessForUser:(LASUser *)user;
...
/*!
 ...
 @param user The user to assign access.
 */
- (void)setWriteAccess:(BOOL)allowed forUser:(LASUser *)user;
...
/*!
 ...
 @return YES if the user has write access. NO otherwise.
 */
- (BOOL)getWriteAccessForUser:(LASUser *)user;
...
/*!
 ...
 @return YES if the role has read access. NO otherwise.
 */
- (BOOL)getReadAccessForRole:(LASRole *)role;
 ...
 */
- (void)setReadAccess:(BOOL)allowed forRole:(LASRole *)role;
 ...
 */
- (BOOL)getWriteAccessForRole:(LASRole *)role;
...
/*!
 ...
 @param role    The role to assign access.
 */
- (void)setWriteAccess:(BOOL)allowed forRole:(LASRole *)role;

/** @name Setting Access Defaults */

/*!
 Sets a default ACL that will be applied to all LASObjects when they are created.
 
 @param acl                 The ACL to use as a template for all LASObjects created after setDefaultACL has been called. This value will be copied and used as a template for the creation of new ACLs, so changes to the instance after setDefaultACL has been called will not be reflected in new LASObjects.
 @param currentUserAccess   If true, the LASACL that is applied to newly-created LASObjects will provide read and write access to the currentUser at the time of creation. If false, the provided ACL will be used without modification. If acl is nil, this value is ignored.
 */
+ (void)setDefaultACL:(LASACL *)acl withAccessForCurrentUser:(BOOL)currentUserAccess;

/*!
 Sets a default ACL that will be applied to all LASObjects when they are created.
 
 @param acl     The ACL to use as a template for all LASObjects created after setDefaultACL has been called. This value will be copied and used as a template for the creation of new ACLs, so changes to the instance after setDefaultACL has been called will not be reflected in new LASObjects.
 @param allowed If true, the LASACL that is applied to newly-created LASObjects will provide read and write access to the `user` at the time of creation.  If false, the provided ACL will be used without modification. If acl is nil or user is unsaved, this value is ignored.
 @param user    The user must already be saved on the server.
 */
+ (void)setDefaultACL:(LASACL *)acl withAccess:(BOOL)allowed forUser:(LASUser *)user;
